...
<?php
	...
	include('assets/inc/config.php');
		if(isset($_POST['admin_sup']))
		{
            ...
            // Get literacy and portal access data
            $ad_literacy = $_POST['ad_literacy']; // 'Literate' or 'Illiterate'
            $ad_access_history = $_POST['ad_access_history']; // 'Yes' or 'No'

            // sql to insert captured values including literacy and history access data
			$query="insert into his_admin (ad_fname, ad_lname, ad_email, ad_pwd, literacy_status, portal_access) values(?,?,?,?,?,?)";
			$stmt = $mysqli->prepare($query);
			$rc=$stmt->bind_param('ssssss', $ad_fname, $ad_lname, $ad_email, $ad_pwd, $ad_literacy, $ad_access_history);
			$stmt->execute();

			//Success/Failure alerts using Sweet Alerts
			if($stmt)
			{
				$success = "Created Account Successfully! Proceed To Log In";
			}
			else {
                        ...
                        <div class="card bg-pattern">
                            ...
                            <div class="card-body p-4">
                                ...
                                <form method='post'>
                                    ...
                                    <div class="form-group">
                                        <label for="literacy">Are you literate?</label>
                                        <select class="form-control" name="ad_literacy" id="literacy" required>
                                    ...
                                    <div class="form-group">
                                        <label for="portal_access">Do you want access to your medical history from the portal?</label>
                                        <div>
                                            <label class="radio-inline">
                                                <input type="radio" name="ad_access_history" value="Yes" required> Yes
                                            </label>
                                            <label class="radio-inline">
                                                <input type="radio" name="ad_access_history" value="No" required> No
                                            </label>
                                        </div>
                                    </div>
